...
def render_login_screen(erro: bool = False):

    """Tela de boas-vindas exibida enquanto a senha não é inserida."""
    ...
    if logo_path.exists():
        ...
        logo_img = f'<img src="data:image/png;base64,{logo_b64}" style="width:420px;height:auto;margin-bottom:28px;filter:drop-shadow(0 0 48px rgba(138,99,210,0.55));" />' 
    else:
        logo_img = '<div style="width:80px;height:80px;border-radius:50%;background:rgba(138,99,210,0.2);border:2px solid rgba(138,99,210,0.4);margin:0 auto 28px;"></div>'
    ...
    .login-badge {{
        ...
        color: #b48ef0; background: rgba(138, 99, 210, 0.10);
        border: 1px solid rgba(138, 99, 210, 0.25);
        border-radius: 999px; padding: 4px 14px; margin-bottom: 22px;
    }}
    .login-badge-dot {{ width: 6px; height: 6px; border-radius: 50%; background: #a070e8; display:inline-block; }}
    .login-title {{
        font-family: "DM Serif Display", Georgia, serif;
        font-size: 2.6rem; font-weight: 400;
        color: #f0ecfc; line-height: 1.1; margin-bottom: 10px;
    }}
    .login-title em {{ font-style: italic; color: #a070e8; }}
    .login-sub {{
        font-family: "DM Sans", sans-serif;
        font-size: 0.95rem; color: #6b5f84; margin-bottom: 0; font-weight: 400;
        max-width: 360px;
    }}
    .login-divider {{
    ...
    <div class="login-wrap">
        {logo_img}
        <div class="login-badge"><span class="login-badge-dot"></span>Pró-Corpo Estética</div>
        <div class="login-title">Análise de <em>Rentabilidade</em></div>
        <div class="login-sub">Painel gerencial de desempenho e resultado financeiro</div>
    ...
    if erro:
        st.sidebar.markdown(
            ...
            '<span style="background:rgba(220,38,38,0.12);border:1px solid rgba(220,38,38,0.30);'
            'border-radius:8px;padding:8px 14px;color:#f87171;font-size:0.8rem;'
