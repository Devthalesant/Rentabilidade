...
def criar_base_final(vmb, log=None):
    """
    ...
    log: callable opcional — ex: status.write ou st.warning.
         Se None, usa print como fallback.
    """
    ...
    colunas_vmb = [
        'ID orçamento', 'Status', 'Data venda', 'Mes_num', 'Ano', 'Unidade',
        'Valor tabela', 'Valor líquido', 'Procedimento', 'Quantidade',
        'Valor liquido item', 'ID cliente', 'Classificação cliente'
    ]
    vmb = vmb[colunas_vmb]
    ...
    vmb[['Valor líquido', 'Valor liquido item', 'Valor tabela']] = (
        vmb[['Valor líquido', 'Valor liquido item', 'Valor tabela']].fillna(0)
    )
    vmb['Valor_unitário'] = np.where(
        vmb['Quantidade'].fillna(0) != 0,
        vmb['Valor liquido item'] / vmb['Quantidade'],
        0
    )

    # Criando coluna para informar a categoria da venda
    vmb['Categoria_da_venda'] = np.where(
        vmb['Procedimento'].str.contains('CORTESIA', case=False, na=False),
        'CORTESIA', 'VENDA'
    )

    # ── NOMENCLATURAS ──────────────────────────────────────────────────────────
    de_para_nomes = carregar_doc_mongo(
        "rentabilidade_anual", "De-Para Nomenclaturas", "DE_PARA_NOMENCLATURAS"
    )
    ...
    if qtd_sem_nomenclatura > 0:
        _log(
            f"❌ Os seguintes procedimentos não têm cadastro no De-Para de Nomenclaturas:\n"
            f"{procedimentos_sem_nomenclatura_padrao}\n\n"
            f"Por favor, cadastre esse(s) procedimento(s) na aba De-Para de Procedimentos."
        )
        return None
    else:
        _log("✅ Nomenclaturas OK")

    # ── TEMPOS ─────────────────────────────────────────────────────────────────
    de_para_tempo = carregar_doc_mongo(
        "rentabilidade_anual", "De-Para Tempo Procedimentos", "DE_PARA_TEMPO_PROCEDIMENTOS"
    )
    ...
    if qtd_sem_tempo > 0:
        _log(
            f"❌ Os seguintes procedimentos não têm cadastro no De-Para de Tempo:\n"
            f"{procedimentos_sem_tempo_padrao}\n\n"
            f"Por favor, cadastre esse(s) procedimento(s) na aba De-Para de Procedimentos."
        )
        return None
    else:
    ...
    # ── CUSTO FIXO ─────────────────────────────────────────────────────────────
    df_custos_fixos = carregar_doc_mongo(
        "rentabilidade_anual", "Custos_Fixos", periodo, asdataframe=True
    )

    if df_custos_fixos.empty:
        _log(
            f"❌ Não temos dados de Custo Fixo para o período {periodo}.\n\n"
            f"Por favor, faça o upload da base na aba Upload de Bases."
        )
        return None
    else:
        _log("✅ Custo Fixo OK")

    # Calculando tempo ocioso e custo da ociosidade
    ...
    df_custos_fixos['Taxa Ociosidade (Min)'] = (
        df_custos_fixos['Custo da Ociosidade'] / df_custos_fixos['Tempo Vendido']
    )
    df_custos_fixos.replace([np.inf, -np.inf], np.nan, inplace=True)

    vmb_custo_fixo = pd.merge(
        vmb,
        df_custos_fixos[[
            'Unidade', 'Minutos Disponivel', 'Tempo Vendido', 'Tempo ocioso',
            'Custo da Ociosidade', 'Taxa Sala (Min)', 'Taxa Ociosidade (Min)'
        ]],
        how='left', on='Unidade'
    )
    vmb_custo_fixo['Custo_fixo'] = (
        (vmb_custo_fixo['Taxa Sala (Min)'] + vmb_custo_fixo['Taxa Ociosidade (Min)'])
        * vmb_custo_fixo['tempo_procedimento']
    )

    # ── CUSTOS PROCEDIMENTOS ───────────────────────────────────────────────────
    de_para_custos = carregar_doc_mongo(
        "rentabilidade_anual", "Custos Procedimentos Mensal",
        f"ALL_COSTS_{ano}_MENSAL", Mes_num
    )

    if not de_para_custos:
        _log(
            f"❌ Não temos dados de Custos Procedimentos Mensal para o período {periodo}.\n\n"
            f"Por favor, faça o cadastro dos custos na aba Custos do Mês."
        )
        return None
    else:
        _log("✅ Custos Procedimentos OK")

    campos_custo = {
        "Custo_Produto": "CUSTO PRODUTO",
        "Custo_MOD": "MOD",
        "Custo_insumos": "CUSTO INSUMOS"
    }
    ...
    for nome_coluna, chave_custo in campos_custo.items():
        ...
        vmb_custo_fixo_produto[nome_coluna] = (
            vmb_custo_fixo["Procedimento_padronizado"].map(mapa) * vmb_custo_fixo["Quantidade"]
        ).fillna(0)

    # ── REORDENAÇÃO E DF AUXILIAR ──────────────────────────────────────────────
    col_order = [
        'Mes_num', 'Ano', 'ID orçamento', 'ID cliente', 'Classificação cliente',
        'Data venda', 'Mês', 'Unidade', 'Procedimento_padronizado', 'Valor tabela',
        'Valor líquido', 'Quantidade', 'Valor liquido item', 'Valor_unitário',
        'tempo_procedimento', 'Minutos Disponivel', 'Tempo Vendido', 'Tempo ocioso',
        'Custo da Ociosidade', 'Taxa Sala (Min)', 'Taxa Ociosidade (Min)',
        'Custo_fixo', 'Custo_Produto', 'Custo_MOD', 'Custo_insumos', 'Categoria_da_venda'
    ]
    vmb_custo_fixo_produto = vmb_custo_fixo_produto[col_order]

    df_tempo_unidade_mes = vmb_custo_fixo_produto[[
        'Ano', 'Mes_num', 'Unidade', 'Minutos Disponivel',
        'Tempo Vendido', 'Tempo ocioso', 'Custo da Ociosidade'
    ]].copy()
    df_tempo_unidade_mes['periodo'] = (
        df_tempo_unidade_mes['Ano'].astype(str) + '-'
        + df_tempo_unidade_mes['Mes_num'].astype(str).str.zfill(2)
    )
    df_tempo_unidade_mes = df_tempo_unidade_mes.drop_duplicates(
        subset=['periodo', 'Unidade']
    ).reset_index(drop=True)

    vmb_custo_fixo_produto = vmb_custo_fixo_produto.drop(
        columns=['Minutos Disponivel', 'Tempo Vendido', 'Tempo ocioso', 'Custo da Ociosidade']
    )

    # ── CUSTOS FINANCEIROS ─────────────────────────────────────────────────────
    df_custos_financeiros = carregar_doc_mongo(
        "rentabilidade_anual", "impostos_taxas", periodo, asdataframe=True
    )

    if df_custos_financeiros.empty:
        _log(
            f"❌ Não temos dados de Impostos e Taxas para o período {periodo}.\n\n"
            f"Por favor, faça o upload da base na aba Upload de Bases."
        )
        return None
    else:
        _log("✅ Impostos e Taxas OK")

    vmb_custo_fixo_produto_fin = pd.merge(
        vmb_custo_fixo_produto,
        df_custos_financeiros[['Mês', 'Custo_Sobre_Venda']],
        how='left', on='Mês'
    )
    ...
    vmb_custo_fixo_produto_fin['Custo_financeiro_produto'] = (
        ...
        * vmb_custo_fixo_produto_fin['Custo_Sobre_Venda']
    )
    vmb_custo_fixo_produto_fin['Custo_financeiro_real'] = (
        vmb_custo_fixo_produto_fin['Valor líquido']
        * vmb_custo_fixo_produto_fin['Custo_Sobre_Venda']
    )

    vmb_custo_fixo_produto_fin = vmb_custo_fixo_produto_fin.drop(
        columns=['Custo_Sobre_Venda']
    ).reset_index(drop=True)
    ...
    vmb_custo_fixo_produto_fin['Lucro_líquido_item_%'] = np.where(
        vmb_custo_fixo_produto_fin['Valor liquido item'] != 0,
        vmb_custo_fixo_produto_fin['Lucro_líquido_item']
        / vmb_custo_fixo_produto_fin['Valor liquido item'],
        np.nan
    )
    ...
    colum_order = [
        'ID orçamento', 'ID cliente', 'Classificação cliente', 'Data venda', 'Mês',
        'Mes_num', 'Ano', 'Unidade', 'Procedimento_padronizado', 'Quantidade',
        'Categoria_da_venda', 'Valor tabela', 'Valor_unitário', 'Valor líquido',
        'Valor liquido item', 'tempo_procedimento', 'Taxa Sala (Min)', 'Taxa Ociosidade (Min)',
        'Custo_Produto', 'Custo_MOD', 'Custo_insumos', 'Custo_financeiro_produto',
        'Custo_financeiro_real', 'Custo_fixo', 'Custo_direto_procedimento',
        'Custo_total_procedimento', 'Lucro_líquido_item', 'Lucro_líquido_item_%'
    ]
    vmb_custo_fixo_produto_fin = vmb_custo_fixo_produto_fin[colum_order]

    _log("✅ Base final gerada com sucesso!")

    return df_tempo_unidade_mes, vmb_custo_fixo_produto_fin